...
export function createMcpServer(): McpServer {
  ...
  server.tool('list_categories', '列出所有文档分类', {}, () => {
    const cats = getDb().prepare('SELECT c.id, c.name, c.description, COUNT(d.id) as doc_count FROM categories c LEFT JOIN documents d ON d.category_id = c.id GROUP BY c.id ORDER BY c.position').all();
    return { content: [{ type: 'text', text: JSON.stringify(cats, null, 2) }] };
  });

  server.tool('list_documents', '列出文档', {
    category: z.string().optional(), status: z.string().optional(), limit: z.number().optional(),
    ...
    return { content: [{ type: 'text', text: JSON.stringify(getDb().prepare(sql).all(...params), null, 2) }] };
  });

  server.tool('get_document', '读取文档全文', { doc_id: z.string() }, ({ doc_id }) => {
    const doc = getDb().prepare('SELECT d.*, c.name as category FROM documents d LEFT JOIN categories c ON d.category_id = c.id WHERE d.id = ?').get(doc_id);
    if (!doc) return { content: [{ type: 'text', text: 'Document not found' }], isError: true };
    return { content: [{ type: 'text', text: JSON.stringify(doc, null, 2) }] };
  });

  server.tool('create_document', '创建文档', {
    title: z.string(), content: z.string(), category: z.string().optional(), project_name: z.string().optional(),
    tags: z.array(z.string()).optional(), status: z.enum(['draft', 'published', 'to_verify', 'archived']).optional(),
    ...
    if (category) {
      ...
      else { categoryId = uuid(); const maxPos = (db.prepare('SELECT MAX(position) as m FROM categories').get() as any)?.m ?? -1; db.prepare('INSERT INTO categories (id, name, position) VALUES (?,?,?)').run(categoryId, category, maxPos + 1); }
    }
    let projectId: string | null = null;
    if (project_name) { const p = db.prepare('SELECT id FROM projects WHERE name = ? COLLATE NOCASE').get(project_name) as any; if (p) projectId = p.id; }
    const id = uuid();
    db.prepare('INSERT INTO documents (id, category_id, project_id, title, content, tags, status) VALUES (?,?,?,?,?,?,?)').run(id, categoryId, projectId, title, content, JSON.stringify(tags || []), status || 'published');
    return { content: [{ type: 'text', text: JSON.stringify({ id, title, message: 'Document created' }) }] };
  });

  server.tool('update_document', '更新文档', {
    doc_id: z.string(), title: z.string().optional(), content: z.string().optional(), category: z.string().optional(),
    tags: z.array(z.string()).optional(), status: z.enum(['draft', 'published', 'to_verify', 'archived']).optional(), pinned: z.boolean().optional(),
    ...
    if (category) {
      ...
      else { categoryId = uuid(); db.prepare('INSERT INTO categories (id, name, position) VALUES (?,?,?)').run(categoryId, category, 0); }
    }
  ...
  server.tool('search_documents', '搜索文档', { query: z.string() }, ({ query }) => {
    const pattern = `%${query}%`;
    const docs = getDb().prepare('SELECT d.id, d.title, d.status, c.name as category, d.updated_at FROM documents d LEFT JOIN categories c ON d.category_id = c.id WHERE d.title LIKE ? OR d.content LIKE ? ORDER BY d.updated_at DESC LIMIT 20').all(pattern, pattern);
  ...
  server.tool('get_overview', '获取全局概览', {}, () => {
    ...
    return { content: [{ type: 'text', text: JSON.stringify({ total_tasks: totalTasks, in_progress: inProgress, done, todo: totalTasks - inProgress - done, overdue_tasks: overdue, total_documents: totalDocs, recent_documents: recentDocs, project_summaries: summaries }, null, 2) }] };
  });

  server.tool('search', '搜索任务和文档', { query: z.string() }, ({ query }) => {
    const pattern = `%${query}%`;
    const tasks = getDb().prepare('SELECT id, title, description, priority FROM tasks WHERE title LIKE ? OR description LIKE ? LIMIT 20').all(pattern, pattern);
    const docs = getDb().prepare('SELECT d.id, d.title, c.name as category FROM documents d LEFT JOIN categories c ON d.category_id = c.id WHERE d.title LIKE ? OR d.content LIKE ? LIMIT 20').all(pattern, pattern);
